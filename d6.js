...
function keepdice(node) {
        if (node.parentNode.id == 'dicekeep') {
//                if ( node.style.filter == 'brightness(40%)' ) {
//                        window.setTimeout(function() {node.style.filter = ''}, 200);
//                } else {
//                        window.setTimeout(function() {node.style.filter = 'brightness(40%)'}, 200);
//                }
                returndice(node)
        } else {
                document.getElementById('dicekeep').appendChild(node.cloneNode(true))
                node.style.display='none'
        }
}

function returndice(node) {
        if (node.parentNode.id == 'dicekeep') {
                document.getElementById(node.id).src=node.src
...
D6AnimBuilder.prototype.genDiceHtml = function(layout, callback, callbackData) {
	...
	for (var i=0; i<layout.length; ++i) {
		...
		genHtml += "<div id='" + this.id + "_diceGroup_" + i + "' class='diceGroup' style='float: left;'>";
		var imgsThisRow = layout[i] * this.groupsize;
		for (var j=0; j<imgsThisRow; ++j) {
			++dieCount;
			if (dieCount > numTotalImgs) break;
			if (this.useImages) {
				genHtml += "<img id='" + this.id + dieCount + "' class='die' src='" + this.baseUrl + "blank.gif' ondblclick='returndice(this)' onclick='keepdice(this)' />";
			} else {
...
D6.dice = function(numDice, callback, callbackData, useImages, buttonLabel) {
	...
	var genHtml = "<hr style='display: none;' /><div id='diceall'>" + builder.genDiceHtml(layout, D6.middleManCallback, middleManData);
	if (buttonLabel != "none") {
		genHtml += "<div id='diceform' style='float: left; margin: 0 6 0 6; padding-top: 40px;'><form><input type='button' id='dicebutton' value='" + buttonLabel + "' onclick='D6AnimBuilder.get(\"dice\").reset(); D6AnimBuilder.get(\"dice\").start()' /></form></div>";
	}
	genHtml += "<div id='dicekeep' style='float: left;'></div>";
	genHtml += "</div><hr style='display: none;' />";
	D6.genHtml = genHtml;
